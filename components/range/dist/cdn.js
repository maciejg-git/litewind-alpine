(() => {
    ...
    Alpine2.data("range", () => {
      return {
        ...
        init() {
          ...
          this._throttledOnMouseMove = Alpine2.throttle(this._onMousemove, 50);
          this.$nextTick(() => {
            this._min = parseFloat(
              Alpine2.bound(this.$el, "data-min") ?? this._min
            );
            this._max = parseFloat(
              Alpine2.bound(this.$el, "data-max") ?? this._max
            );
            this._step = parseFloat(
              Alpine2.bound(this.$el, "data-step") ?? this._step
            );
            this._fixedMin = JSON.parse(
              Alpine2.bound(this.$el, "data-fixed-min") ?? this._fixedMin
            );
            this._showSteps = JSON.parse(
              Alpine2.bound(this.$el, "data-show-steps") ?? this._showSteps
            );
            let showLabels = Alpine2.bound(this.$el, "data-show-labels") ?? this._showLabels;
  ...
  // components/range/builds/cdn.js
  document.addEventListener("alpine:init", () => {
    Alpine.plugin(range_default);
  });
})();
