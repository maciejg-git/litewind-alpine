(() => {
  ...
  function validation_default(Alpine2) {
    ...
    Alpine2.directive(
      "validation",
      (el, { value, expression }, { Alpine: Alpine3, evaluateLater, cleanup }) => {
        ...
        if (Alpine3.$data(el).formName === void 0) {
          return;
        }
        Alpine3.$data(el).addInput({
          name: inputName,
          status: {},
          messages: {},
          state: ""
        ...
        let validation = useValidation(
          ...
          (res) => {
            Alpine3.$data(el).inputs[inputName].status = res.status;
            Alpine3.$data(el).inputs[inputName].messages = res.messages;
            Alpine3.$data(el).inputs[inputName].state = res.state;
          }
        );
        let getter = () => {
          let value2;
        ...
        cleanup(watchValue);
        cleanup(() => Alpine3.$data(el).removeInput(inputName));
      }
    );
    Alpine2.magic("validation", (el, { Alpine: Alpine3 }) => (input) => {
      return Alpine3.$data(el).inputs[input];
    });
  }
